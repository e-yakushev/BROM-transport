    ...
    subroutine calculate_phys(i_water, k_max, par_max,  cc, fick, dcc, use_bound_up, use_bound_low, bound_up, bound_low, &
                            surf_flux, k_bbl_sed, dz, kz, kz_bio, kz_tot, julianday,  i_O2,  dt, freq_turb)
    
    !subroutine calculate_phys(cc, fick, dcc, k_max, par_max, use_bound_up, use_bound_low, bound_up, bound_low, &
    !                        surf_flux, k_bbl_sed, kz_tot, kz, julianday, kz_bio, i_O2, dz, dt, freq_turb)
    ...
    do k = 2, (k_max - 1)
        ...
        do ip = 1, par_max
            ...
            !if (k > k_bbl_sed - 1) then
            !    kz_tot(k) = (kz(k,  julianday) + kz_bio(k)                                             &
            !        * cc(k_bbl_sed - 1, i_O2) / (cc(k_bbl_sed - 1, i_O2) + 5.))!for plots
            !else
            !    kz_tot(k) = 0.
            !end if 
            ...
            fick(i,k, ip) = (kz(i,k, julianday) + kz_bio(k)                                                &
                * cc(i,k_bbl_sed - 1, i_O2) / (cc(i,k_bbl_sed - 1, i_O2) + 5.))   &
                * (cc(i,k + 1, ip) - cc(i,k, ip)) / dz(k)
            !fick_bio(i,k, ip) = ( kz_bio(k)                                                &
            !    * cc(i,k_bbl_sed - 1, i_O2) / (cc(i,k_bbl_sed - 1, i_O2) + 5.))   &
            !    * (cc(i,k + 1, ip) - cc(i,k, ip)) / dz(k)
            ! fick_phys(i,k, ip) = (kz(i,k, julianday) )   &
            !    * (cc(i,k + 1, ip) - cc(i,k, ip)) / dz(k)           
    ...
    do k = 2, (k_max - 1)
        do ip = 1, par_max
            cc(i,k, ip) = cc(i,k, ip) + (dt / freq_turb) * dcc(i,k, ip)
        end do
    end do
...
!    subroutine calculate_sed(par_max, k_max, kz, julianday, wbio, cc, dcc, dz, dt, freq_sed)
    ...
    real(8)                                             :: w_u(par_max)
    real(8)                                             :: w_d(par_max)
    real(8), parameter                                  :: w_burying = 15.e-6 ! rate of burying 5*0.1 cm/y
    integer                                             :: i, k, ip
